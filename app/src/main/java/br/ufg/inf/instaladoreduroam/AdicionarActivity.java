...
public class AdicionarActivity extends ActionBarActivity {
    ...
    private static final String LOG_TAG = AdicionarActivity.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_adicionar_eduroam);
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.menu_adicionar_eduroam, menu);
        return true;
    }
        ...
        if (networks != null) {
            if (networks.size() > 0) {
                ...
                for (WifiConfiguration config : networks) {
                    if (config.SSID.equals(Configuracoes.SSID_NAME)) {
                        foundConfig = config;
                        break;
                    }
                }
    ...
    private void adicionarRedeEduroam() {
        ...
        /*AP Name*/
            selectedConfig.SSID = Configuracoes.SSID_NAME;
        ...
        /*Protocols*/
            ...
            try {
                ...
                for (Class wcClass : wcClasses)
                    if (wcClass.getName().equals(Configuracoes.INT_ENTERPRISEFIELD_NAME)) {
                ...
                for (Field wcefField : wcefFields) {
                    if (wcefField.getName().equals(Configuracoes.INT_ANONYMOUS_IDENTITY))
                        wcefAnonymousId = wcefField;
                    else if (wcefField.getName().equals(Configuracoes.INT_CA_CERT))
                        wcefCaCert = wcefField;
                    else if (wcefField.getName().equals(Configuracoes.INT_CLIENT_CERT))
                        wcefClientCert = wcefField;
                    else if (wcefField.getName().equals(Configuracoes.INT_EAP))
                        wcefEap = wcefField;
                    else if (wcefField.getName().equals(Configuracoes.INT_IDENTITY))
                        wcefIdentity = wcefField;
                    else if (wcefField.getName().equals(Configuracoes.INT_PASSWORD))
                        wcefPassword = wcefField;
                    else if (wcefField.getName().equals(Configuracoes.INT_PHASE2))
                        wcefPhase2 = wcefField;
                    else if (wcefField.getName().equals(Configuracoes.INT_PRIVATE_KEY))
...
//                    wcefEngineId = wcefField;
        ...
        /*EAP Method*/
                if (!noEnterpriseFieldType) {
                    wcefSetValue.invoke(wcefEap.get(selectedConfig), Configuracoes.ENTERPRISE_EAP);
                } else {
                    wcefEap.set(selectedConfig, Configuracoes.ENTERPRISE_EAP);
                }
        /*EAP Phase 2 Authentication*/
                if (!noEnterpriseFieldType) {
                    wcefSetValue.invoke(wcefPhase2.get(selectedConfig), Configuracoes.ENTERPRISE_PHASE2);
                } else {
                    wcefPhase2.set(selectedConfig, Configuracoes.ENTERPRISE_PHASE2);
                }
